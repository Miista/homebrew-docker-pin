...
// The decider's log is meant to be read by a person looking at `docker logs`
...
//
// The timestamp carries the date. This process is long-lived and hears from
// the detector once a day, so a bare clock time cannot tell yesterday's
// notification from today's -- and `docker logs` on a container up for a week
// is exactly where that question gets asked.

// newLogger builds the process-wide logger. Level comes from
// DECIDER_LOG_LEVEL, so verbosity can be turned up on a running host without
