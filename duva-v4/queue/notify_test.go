...
func TestTheMessageIsTheServiceAndTheChange(t *testing.T) {
	title, body := message("optiplex", queue.Entry{
...
func TestADigestMoveSaysWhatMoved(t *testing.T) {
	_, body := message("optiplex", queue.Entry{
...
// A queue with no host still says something useful.
func TestAMissingHostLeavesTheTitleAsTheService(t *testing.T) {
	title, _ := message("", queue.Entry{Service: "whoami", From: "1.0", To: "1.1", Kind: ociversion.KindMinor}, queued, "")
	if title != "whoami" {
...
func TestAnAnnouncementCarriesTheTopicTitleAndButton(t *testing.T) {
	...
	n.announce("optiplex", queue.Entry{
		Service: "whoami", From: "v1.10.4", To: "v1.12.0", Kind: ociversion.KindMinor,
	}, queued, "")
...
func TestAnUnreachableNotifierIsNotFatal(t *testing.T) {
	...
	n.announce("optiplex", queue.Entry{Service: "whoami", From: "1.0", To: "1.1", Kind: ociversion.KindMinor}, queued, "")
...
// A failed auto-apply reads as one, and carries the updater's reason.
//
