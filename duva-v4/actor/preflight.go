...
// The actor's startup preconditions.
//
...
//
// Refusing to start is right here, where it would be wrong for the detector.
// A detector whose webhook is down still detects and logs; an actor that
...
// preflight checks everything the actor needs before it agrees to serve.
//
