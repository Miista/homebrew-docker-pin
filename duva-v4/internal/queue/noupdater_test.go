...
// Clicking Update with no updater configured must say so, not panic. The
...
// An updater that is there but would refuse must reach the page as not-applyable,
// with its own words. The reference updater refuses on an uncommitted
...
// The positive: a ready updater reports applyable with nothing to explain.
...
// style them apart without matching on prose the updater is free to reword.
...
// A ready updater carries neither, so the page renders no banner at all.
