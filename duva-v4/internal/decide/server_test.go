...
// A detector reaches this over the host's own network and carries no
...
// The decider is a pipe: what the actor said is copied through unchanged, so
// the actor keeps owning the shape of its own progress.
