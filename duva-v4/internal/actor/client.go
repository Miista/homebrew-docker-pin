...
// Client is how a decider reaches an actor.
//
// It knows an address, a token, and the contract. It does not know what
// applying means, which is the point: a different actor at the same address
// answering the same two calls is indistinguishable from this one.
type Client struct {
	// BaseURL is the actor's root, e.g. http://actor:8080.
	BaseURL string
	// Token authenticates the decider to the actor. They share a network, so
	...
	// stream is long-lived by design — bounding it is the decider's job,
...
// Apply asks the actor to do the work and returns where to watch it.
func (c *Client) Apply(ctx context.Context, r Request) (Accepted, error) {
	...
	// actor that cannot even accept a request is not one to wait on.
...
//
// One connection serving two readers: the decider needs to know when the work
// ended, and a person watching the UI needs to see it happen. Opening the
// stream twice would ask the actor to render the same work to two audiences,
// and give them different views of it if the second connected late.
//
// tee may be nil, for a decider applying something nobody is watching.
//
// The returned status is what the actor said. A stream that ends without
...
func (c *Client) Watch(ctx context.Context, streamURL string, tee io.Writer) (Status, string, error) {
	...
	// The actor stopped talking without saying how it went. That is a
...
// absolute resolves a stream location against the actor's base.
//
// The actor may answer with either, and neither is wrong: a path is what it
// knows for certain, and an absolute URL is what an actor behind something
