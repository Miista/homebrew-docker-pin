...
// the queue's business. Misreading one as the end would cut a stream short.
...
// one as the end would let a typo in an updater read as success; leaving it as
// progress means the queue waits and its timeout catches it.
...
// contract: a field renamed here is a different updater's parse failing.
