...
//
// A webhook, not "the queue". The watcher does not know what is at the
...
//
// That is why the payload is the watcher's own shape rather than any
...
type reporter struct {
	...
	// needs. One unreachable webhook must not cost the watcher its ability
	// to watch.
	//
...
func newReporter(url, host string, log zerolog.Logger) *reporter {
	...
	if url != "" {
		if err := dialWebhook(url); err != nil {
			...
			// start: a watcher whose webhook is down still logs what it
...
// Event is what the watcher publishes: one tag, and when it appeared.
//
// Everything here is something the watcher observed. There is no "current
...
// be a request per finding to tell a consumer something the updater learns for
...
type Event struct {
	// Watcher names the tool and its version, so a consumer reading a
	...
	// ObservedAt is when this watcher noticed, which is not the same thing:
...
//
// No webhook configured is a failure to hand on, not a success. A watcher
...
func (r *reporter) report(f watch.Finding) bool {
	if r.url == "" {
		return false
	}
	...
	// which is how a watcher becomes a flood.
	if r.unreachable {
		r.skipped++
		r.failed++
		return false
