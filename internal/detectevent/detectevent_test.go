...
// The payload the detector actually sends, verbatim rather than reduced to
// the fields used: if the detector changes shape, the test that notices
...
func TestParseAndTranslate(t *testing.T) {
	...
	// tell the gate something the actor learns when it pulls.
	if n.Digest != "" {
...
// Unknown fields are ignored rather than rejected: the detector may add to
