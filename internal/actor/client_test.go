...
// fakeActor is an HTTP server answering like an actor, so the client is
...
type fakeActor struct {
	...
	stream string
	// got records the request the actor was given.
	got Request
...
// An actor that accepts work but says nowhere to watch it has broken the
// contract, and pretending otherwise means a decider waiting on nothing.
func TestApplyRejectsAnEmptyStream(t *testing.T) {
	// An actor answering with an explicitly empty stream field.
...
// the actor said must reach the watcher, terminal line included -- a person
...
// An empty stream is the same case, and is what the actor serves when nothing
// is running.
...
// The timeout is the decider's, expressed as a context -- it is what keeps an
// actor that died mid-apply from wedging the queue forever.
...
// The actor may answer with a path or a full URL, and neither is wrong: a
// path is what it knows for certain, an absolute URL is what an actor behind
