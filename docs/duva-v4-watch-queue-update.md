# duva v4: diun, queue, updater

**Status: built and working end to end**, on `feature/duva-v4-queue` —
`internal/decide`, `internal/updater`, `internal/diun`, `internal/dockerapi`,
`cmd/queue`, `cmd/updater`. Not deployed. Existing duva is untouched and
still running.
...
was verified first by accident — the updater could not find its compose
...
| | question | who |
|---|---|---|
| **watcher** | has a new matching version been published? | **diun** (exists, deployed) |
| **queue** | does it fit, how big is it, may it be applied? | new, small |
| **updater** | rewrite the file, replace the container, commit | duva's `apply.go`, extracted |

Plus a **UI**, which is a view over the queue's queue with one write verb.

```
diun ──webhook──▶ queue (the gate)
                     ├── auto ──▶ updater
                     └── pending ──▶ [queue] ◀── reads ── UI
                                        └── approve ──▶ updater
```

## Why diun is the watcher
...
watcher seam is different — it has a concrete occupant today.
...
If diun is the watcher, those labels are **correct as they stand** and nothing
needs migrating.
...
The queue therefore needs **no registry access at all** — no tag listing, no
...
| watcher | — | yes | — | — |
| queue | — | yes | — | — |
| updater | read-write | yes | yes | yes |
| UI | — | — | — | — |

(This document argues below for diun as the watcher. That did not survive
contact: diun watches whole repositories, which made every run a tag-listing
flood and a lost database an expensive rebuild. The watcher is ours, and it
holds one timestamp. See `duva-v4/README.md` for what actually runs.)

The updater's row has a deployment consequence: it must run **as the user that
...
**Neither tool talks to a registry.** The updater pulls through the daemon —
...
what the queue needs. They should move somewhere neutral during the
extraction rather than dragging the queue into importing a package it uses
five percent of.

**Nothing holds more than it must.** duva today gives every part the socket,
because any part might need to act. Here only the updater does.
...
queue gets the current tag from what it already knows: **the compose file**.
That is the same source duva uses, and it means the queue is not purely
...
5. Emit to the updater (automatic) or hold in the queue (request).

Only the updater writes the compose file. The queue reads it. One of the three
can corrupt state, not three.

## The queue is the gate

Everything passes through it. The UI reads; the updater obeys. Two things follow.

**The queue lives in the queue, not the UI.** A pending update is a decision
awaiting confirmation, which is policy, and policy is the queue's. The UI can
...
The updater locks while applying or while the queue is being updated. If a new
...
**The updater does not revert.** Everything that can be answered before anything
...
updater cannot tell which it is in. An earlier version restored the pin under the
...
This makes "the updater is dumb" true only about *policy*. It is not dumb about
...
**diun, queue and updater are all same-host.** They reach each other over that
host's compose network by name. Only the UI crosses hosts, and it only ever
talks to the queue.

So there is **one published port per host** — the queue's. The updater is
...
`/v1/refresh` goes away — the queue checks nothing, diun does — and
`/v1/notify` arrives.

`/v1/notify` is unauthenticated because diun reaches the queue by container
...
digest, and the queue validates both against its own compose files. An
...
today — before anything reaches the updater. So the worst case is a forged
...
Internal only. The queue calls it; nothing else can.

**The contract is specified in `docs/updater-contract.md`** — endpoints, payloads,
the terminal line format, and what a new updater has to get right. What follows
here is why it is shaped that way.

The contract is deliberately small, because **a different updater must be able
to take its place**. The queue may know an endpoint, a payload — which
...
git: an updater that opens a pull request touches none of them.
...
updater's idea of applying, and a pull-request updater never changes the compose
file, so the queue would re-notify forever.

Instead the updater reports its own completion, and the queue runs a timeout.
**Completed means the updater did its job** — not that the host is running a new
image. A pull request updater completes when the pull request exists. The
timeout is what keeps an updater that died mid-apply from wedging the queue, and
it is the only thing the queue needs to believe about work it did not do.
...
it. The queue **tees** — one connection to the updater, relayed verbatim to
...
stream has two readers with different needs — the queue, which must
...
would let a typo in an updater read as success, where leaving it as progress
...
with a compatibility obligation — which means the queue and the updater are
...
Nothing yet tells a queue which contract version an updater speaks — today it
...
start on an unwritable `/data`. Not worth building with one updater, but worth
leaving room for.

`POST /v1/apply` starts the transaction and returns a URL to stream progress
from. **The queue proxies that stream** — a clean passthrough, copying bytes
without interpreting them.

The passthrough is what preserves the property worth having. The updater still
owns the shape of its own progress: a different updater — one that opens a pull
request, or defers to Ansible, which is what v3 imagined this seam buying —
streams whatever it likes, and the queue relays it unchanged. The queue is
a pipe, not a participant, and learns nothing about what an apply looks like.

Proxying rather than redirecting also means the updater never needs to know its
...
base URL, because nothing outside the host ever addresses the updater.
...
- **The queue notifies when an update *should be applied*,** not when it
  becomes available. Soak, policy and approval resolve on the queue's side;
  the updater is told about work already cleared to happen.
- **Re-notify until the compose file shows the target digest.** The updater needs
...
- **The updater must therefore be idempotent** — told twice about the same update
...
The updater must run where the socket and the compose file are, so queue and
updater are per host. diun already is.
...
```
  host A:   diun ──▶ queue-A ──▶ updater-A
  host B:   diun ──▶ queue-B ──▶ updater-B

                 UI ──reads──▶ queue-A
                    ──reads──▶ queue-B      one page, every queue
```

The UI stays a separate process rather than moving inside the queue. It is
only a thin view on the queue, which is an argument for folding it in — but the
queue is per host, so a UI inside it means one page per host, which is the
...
claim to be a queue, to solve a problem a two-line config solves.
...
| updater | `cmd/duva/apply.go`, `transaction.go`, `recreate.go` |
| UI | `internal/ui` |
| UI transport | `internal/agent`, `internal/hub` — queues replace agents |

Genuinely new: a webhook receiver, the service-mapping rule, the lock, and
the updater's internal HTTP surface with its progress stream.
...
queue reads the compose file for the current tag.

## Mapping a webhook to a service

Settled by the payload above: **`metadata.ctn_names`**.

The queue already parses the compose files to read the current pinned tag,
...
**The queue therefore needs no docker socket.** Compose files in, decisions
out. Only the updater talks to docker, which is a better privilege story than
...
invisible. A webhook naming a container the queue cannot map must be a loud
...
one transaction**. That is the updater, and it is the piece worth keeping
