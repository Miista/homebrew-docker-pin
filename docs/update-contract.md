...
What a queue requires of an updater, so a different one can be written.

`duva-v4/updater` is the reference implementation — it pulls, rewrites the pin,
recreates the container and commits. It is not the only possible one. An updater
...
mean reading the reference updater's source to guess at what is required.

The whole of what the queue knows is below. If something is not here, the
queue does not rely on it, and an updater may do anything it likes about it.
...
registry, a container, a working tree — belongs to the updater and the queue
must never learn about it.

**Completed means the updater did its job**, not that the host is running a new
image. A pull-request updater completes when the pull request exists. This is
...
The updater is reached by the queue on its own host network. It is not
...
when the updater was given one.
...
| `file` | the compose file it is declared in — not necessarily the project root, since `include:` is resolved. Carried so the updater need not re-derive what the queue already knows. |
...
| `digest` | what to end up on, when the queue knows it. Empty for a version change, where the updater resolves it by pulling the tag. |
...
`stream` may be absolute or relative to the updater's base. **The queue
follows it and never constructs it**, so an updater is free to key its streams
...
here means the work never began, and the queue leaves the entry queued.
...
(That is the reference updater's wording, shown to make the shape concrete. None
of it is required — only the last line is.)

Unauthenticated in the reference updater, deliberately: it returns nothing when
...
the update anyway. An updater may guard it if it prefers.

The stream has two readers with different needs — the queue, which must
...
queue does not parse it. Write whatever a person would want to read.
...
This exists because a UI that offers a button for work the updater would refuse
is worse than one that greys it — the person has already decided by the time
they find out. The reference updater answers `false` when its repository has
uncommitted changes, because applying commits and committing on top of
somebody's half-finished edit is never wanted.

Answer `{"ready": true}` if there is nothing that would stop you. An updater
...
reference updater's reason, and the contract must not learn it — an updater that
opens a pull request has no working tree to be dirty. What every updater can
answer is whether it would take work.

Answer it **per call**, not from a cache. What blocks an updater usually stops
...
## What the queue does about all this

It calls `/v1/apply`, follows the `stream` it is given, tees the lines to
whoever is watching, and waits for a terminal line.

It runs a **timeout** — `DUVA_QUEUE_UPDATE_TIMEOUT`, 30 minutes by default. That
is the only thing it has to believe about work it did not do: an updater that
...
decision. If the apply fails, the watcher reports the tag again and it comes
back — which is the protocol working, not state being lost.

The queue never retries an apply itself. Retry is a property of the watcher
running again.
...
is what lets the queue and the updater be released independently rather than
in lockstep.

- **Adding a field is backwards compatible.** An older updater ignores what it
  does not recognise; an older queue ignores what it is sent.
...
actually done your job**, whatever that means for you. The queue believes
you, and the entry is gone from the queue either way.

## Deployment, for an updater that touches a repository

Not part of the contract — an updater that touches no repository can ignore all
of it — but the reference updater carries requirements worth knowing about
