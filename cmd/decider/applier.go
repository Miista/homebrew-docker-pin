...
// Start begins applying an entry, returning once the actor has accepted it.
//
...
// actor did.
func (a *applier) Start(e decide.Entry) error {
	a.watchers.Lock()
...
func (a *applier) apply(e decide.Entry, l *live) {
	defer func() {
		l.finish()
		a.watchers.Lock()
	...
	acc, err := a.client.Apply(ctx, actor.Request{
		Service: e.Service,
		File:    e.File,
		Image:   e.Image,
		From:    e.From,
...
// Write makes live an io.Writer, so the actor client can tee straight into it.
